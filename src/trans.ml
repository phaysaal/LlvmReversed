...
let get_call lli =
  if is_cast lli then
    begin
      ...
      F.dbgf "CALL" "%s :: Casted from: %s" (stv lli) (stv casted_from);
      casted_from
    end
  else
    lli
       ...
       begin  
         ...
         let t_fname = B._T @@ fname in
           ...
           begin
             ...
             let pp = if not (Deps.mem sname !ref_assert_map) then B.SKIP else B.call ~ret:(Some v) t_fname params in 
...
and get_ptr_element vars lli =
  ...
  if L.num_operands lli < 2 then
    ...
    begin
      ...
      F.dbgf "GEP" "|op|:%d op1:%s" (L.num_operands src_ptr) (stv @@ L.operand src_ptr 0) ;
    ...
    begin let lli0 = L.operand lli 0 in
       ...
       begin
         ...
         if LA.is_array lli0ty || LA.is_struct lli0ty then
           ...
           (* F.pn "*** ***"; F.iterS (B.pprint 0) "" ps; F.pn "*** ***"; *)
           ps, ptr'
...
(*
    let f1 k1 =
      F.dbgf "INTRP" "Parse Break Then:: checking Label %s in %d" lblEl k1;
      next_lbl_is blk_arr lblEl (k1,n);
      F.dbgf "INTRP" "Parse Break Then:: Label %s OK" lblEl ;
      begin
        let f k =
          if k<n then
...
and parse_empty_cond (blk_arr, vars, f) (pre_seg, (inst0,lblC), (i,n)) =
  ...
  | None -> F.dbgf "LOOP" "Inter Not OK";
          raise Not_parsed
  (* let r = get_breaks lblR (i,n-1) in
  F.dbgf "LOOP" "r len is %d" (List.length r);
  let v'2, rest_prog = parse_program (blk_arr, vars, f) (j,n) in
  Some (v'2, rest_prog) *)
      ...
      try
        ...
        if lblEl' = lblEl then
          begin
            ...
            let ps2, cond2 = to_cond vars _A in(** TODO: ps2 should be replaced by the program of blk i **)
...
let parse_func vars' name f =
  ...
  if n = -1 then
    ...
    begin
      ...
      let structs = (*! List.fold_left (fun ss ((snm,flds) as s) -> VR.add (F.corr_structname snm) (convert_struct s) ss) VR.empty !structures in *) get_structures () in
  ...
  if num_op = 0 then
    ...
    begin
      ...
      for i=0 to num_op - 1 do
        ...
        in
                  (* let r = get_init_data vars opi in *)
        d := !d @ [r]
      done;
      if num_op = 1 then
        List.hd !d
