...
let rec body_to_cexp ((dep_map, gvs, (vs:S_E.t)) as gvars) prog =
  ...
  | IF (a, b, c, y, l) ->
     ...
     let p = C.Seq (ldp, mk_cond ptrs vs b' c' a, y') in
     (ptrs,
      declared,
      rb @ rc @ ry,
      p)
  ...
  | PROCCALL (_, T.EXP (E.VAR ("printf",_)), [], _,
              y, _)
...
let translate_to_consort consort consortdir fname progs =
  ...
  if consortdir = "" then
    ...
    begin
      let fname' = consortdir ^ fname ^ ".consort" in
      ...
      if consort = "" then
        begin
          F.pn ("It will not be verified by ConSORT since the path is not configured in .conf file");
          ()
        end
      else
        begin
          F.pn ("It will be verified by " ^ consort);
          let s1 = if !interp then
                     Sys.command (consort ^ " -exec interp " ^ fname')
                   else
                     Sys.command (consort ^ " " ^ fname') in
