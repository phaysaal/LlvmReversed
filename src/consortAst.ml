...
let rec simplify_expr ?next count e : pos * A.raw_exp =
  ...
  | Alias (i,v1,v2) -> 
    ...
    |> tag_with i
  | Assert (i,{ rop1; cond; rop2 }) ->
    lift_to_imm ~ctxt:i count (imm_op_to_lhs rop1) (fun c op1' ->
        lift_to_imm ~ctxt:i c (imm_op_to_lhs rop2) (fun c' op2' ->
